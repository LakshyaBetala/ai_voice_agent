...
# Watchdog: kill calls that exceed this many seconds past the 360s cap.
...
def check_runaway(*, started_at_monotonic: float, now: float | None = None) -> RunawayDecision:
    ...
    Two stages:
      - HARD_CAP_SECONDS = 360: the contractual maximum. Soft-close is
        attempted at 350 in the pipeline; if the LLM is still going at
        360, we ask it to wrap and start the watchdog.
      - 360 + RUNAWAY_GRACE_SECONDS: terminate unconditionally.
    """
